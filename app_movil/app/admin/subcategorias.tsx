...
export default function AdminSubcategoriasScreen() {
  ...
  const fetchSubcategorias = async () => {
    ...
    try {
      ...
      Alert.alert('Error', (error as { message?: string })?.message || 'Error al cargar subcategorías');
    } finally {
      setLoading(false);
    }
  };
  ...
  const categoriasFiltradas = useMemo(() => {
    const searchTermLower = searchCategoriaTerm.toLowerCase().trim();
    return categorias
  ...
  const handleCreate = async () => {
    ...
    if (descripcion.trim().length > 250) {
      Alert.alert('Error', 'La descripción de su subcategoría no puede tener más de 250 caracteres');
      return;
    }
          ...
          {isAdmin ? (
            ...
            <View style={styles.infoCard}>
              ...
              <Text style={styles.infoText}>
                Los auxiliares pueden ver subcategorías, pero no crear nuevas.
              </Text>
            </View>
          )}
