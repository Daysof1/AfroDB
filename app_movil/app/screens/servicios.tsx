...
import { FlatList, Modal, Image, ImageBackground, Pressable, RefreshControl, ScrollView, StyleSheet, TextInput, View, ActivityIndicator, Dimensions } from "react-native";
...
// ─────────────────────────────────────────────────────────────
// FUNCIÓN AUXILIAR (extraída del componente)
// ─────────────────────────────────────────────────────────────
...
type ListHeaderProps = {
    ...
    fetchServicios: (page: number, search: string) => void;
...
const ListFooter = ({ loading, hasServicios, totalPaginas, paginaActual, setPaginaActual }: ListFooterProps) => {
    if (loading || !hasServicios || totalPaginas <= 1) {
...
export default function ServiciosScreen() {
    ...
    const serviciosFiltrados = useMemo(() => {
        ...
        return servicios.filter((p: Servicio) => {
    ...
    const handleAgendarCita = (servicio: Servicio) => {
        setServicioSeleccionado(servicio);
        router.push('/agendar');
    };

    const renderServicio = ({ item: servicio, index }: { item: Servicio; index: number }) => (
        <View
            style={[
                styles.card,
    ...
    return (
        <>
            <FlatList
                ...
                keyExtractor={(item: Servicio) => String(item.id)}
                numColumns={2}
