// Página: AuxiliarCategorias.js. gesti?n de categor?as por auxiliar.
import { useEffect, useState } from 'react';
import '../Admin.css';
import { apiRequest } from '../../api/client.js';
import { exportarCategoriasAPDF, exportarCategoriasAExcel } from '../../utils/exportUtils.js';
...
export default function AuxiliarCategorias() {
  ...
  useEffect(() => {
    ...
    const coincideEstado =
      filtro === 'Todos' ||
      (filtro === 'True' && cat.activo) ||
      (filtro === 'False' && !cat.activo);

    return coincideBusqueda && coincideEstado;
  });

  setFilteredCategorias(filtered);
}, [searchTerm, categorias, filtro]);
  ...
  return (
    <div className="admin-page">
      ...
      <button
        ...
        >
          Activas ({categorias.filter((c) => c.activo === true).length})
        </button>
        ...
        >
          Inactivas ({categorias.filter((c) => c.activo === false).length})
        </button>
      </div>
      ...
      {isFormOpen && (
        <div className="form-container">
          ...
          <form onSubmit={handleCrear}>
            ...
            <button type="submit" className="btn btn-primary">{editingCategoriaId ? 'Actualizar' : 'Guardar'}</button>
          </form>
        </div>
      )}
